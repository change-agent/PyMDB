...
while line_no < len(line):
    ...
    #It is assumed that "date" is when the movie was released; therefore, re-released movies will have a new date.
    ...
    for word in keywords_split:
        keywords_split = [k.replace(word, word.strip().lower()) for k in keywords_split]
        while word_no < len(keywords_split):
        #IN PROGRESS - not fully functioning
        #If key is already in the index, append the filename to current key; otherwise, create a new key and mapping.
            if keywords_split[word_no] in index:
                filename_set = index[keywords_split[word_no]]
                filename_set.update(filename)
